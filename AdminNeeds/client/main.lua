...
AddEventHandler('relisoft_tag:owned',function(own,group)
    ...
    if own then
        ownedGroup = group
		exports['mythic_notify']:DoHudText('success', 'Tag Enabled!')
    else
		exports['mythic_notify']:DoHudText('error', 'Tag Disabled!')
    end
end)
...
local sleep = 500
        ...
        if owned then
            sleep = 5
            DrawText3D(cx,cy,cz, 'ADMIN') -- Here you can Edit the "Admin" ---> "Administrator" or "Staff"
        end
...
AddEventHandler('relisoft_players:drawText',function()
    if draw then
        draw = false
        exports['mythic_notify']:DoHudText('error', 'ID Disabled!' )
    else
        draw = true
        exports['mythic_notify']:DoHudText('success', 'ID Enabled!' )
    end
end)
